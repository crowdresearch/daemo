...
class CanCreateAccount(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if view.action == 'create' and not (request.user.is_staff or settings.REGISTRATION_ALLOWED) and users == 0:
            raise PermissionDenied(detail='We are currently in closed beta. If you\'d like an account, email daemo@cs.stanford.edu with a short description of what you\'d like to use Daemo for.')
        return True
