                   ...
                   FROM crowdsourcing_workerrequesterrating wrr
                     INNER JOIN (
                                  SELECT
                                    ...
                                    COUNT(recent_req_rating.target_id) number_of_ratings
                                  FROM (
                                         SELECT
                                           wrr.weight,
                                           wrr.target_id
                                         FROM crowdsourcing_workerrequesterrating wrr
                                           INNER JOIN (
                                             ...
                                             ON most_recent.origin_id = wrr.origin_id AND most_recent.target_id = wrr.target_id
                                                AND wrr.last_updated = most_recent.max_date AND wrr.target_id = $1
                                                AND wrr.origin_type = 'requester') recent_req_rating
                     ...
                     INNER JOIN (
                       ...
                       ON wrr.origin_id = most_recent.origin_id AND wrr.target_id = most_recent.target_id AND
                          wrr.last_updated = most_recent.max_date AND wrr.origin_type = 'requester') worker_ratings
            $$
            LANGUAGE SQL
            STABLE
